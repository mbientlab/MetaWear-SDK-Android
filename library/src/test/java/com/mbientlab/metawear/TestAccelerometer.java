...
public class TestAccelerometer extends UnitTestBase {
    ...
    @ParameterizedTest
    @MethodSource("data")
    public void setOdrCommand(Class<? extends Accelerometer> accelClass) throws InterruptedException {
    ...
    @ParameterizedTest
    @MethodSource("data")
    public void setOdrValue(Class<? extends Accelerometer> accelClass) throws InterruptedException {
        CountDownLatch doneSignal = new CountDownLatch(1);
        setup(accelClass).addOnSuccessListener(IMMEDIATE_EXECUTOR, task -> {
    ...
    @ParameterizedTest
    @MethodSource("data")
    public void setRangeCommand(Class<? extends Accelerometer> accelClass) throws InterruptedException {
        CountDownLatch doneSignal = new CountDownLatch(1);
        setup(accelClass).addOnSuccessListener(IMMEDIATE_EXECUTOR, task -> {
            byte[] expected= null;
    ...
    @ParameterizedTest
    @MethodSource("data")
    public void setRangeValue(Class<? extends Accelerometer> accelClass) throws InterruptedException {
        CountDownLatch doneSignal = new CountDownLatch(1);
        setup(accelClass).addOnSuccessListener(IMMEDIATE_EXECUTOR, task -> {
            float expected= -1, actual;
    ...
    @ParameterizedTest
    @MethodSource("data")
    public void subscribeAccStream(Class<? extends Accelerometer> accelClass) throws InterruptedException {
        CountDownLatch doneSignal = new CountDownLatch(1);
        setup(accelClass).addOnSuccessListener(IMMEDIATE_EXECUTOR, task -> {
            ...
            accelerometer.acceleration().addRouteAsync(source -> source.multicast()
                    ...
                    .index(0).stream(null)
                    .index(1).stream(null)
                    .index(2).stream(null)).addOnSuccessListener(IMMEDIATE_EXECUTOR, ignored -> {
                        assertArrayEquals(expected, junitPlatform.getLastCommand());
                        doneSignal.countDown();
            });
        });
    ...
    @ParameterizedTest
    @MethodSource("data")
    public void unsubscribeAccStream(Class<? extends Accelerometer> accelClass) throws InterruptedException {
        CountDownLatch doneSignal = new CountDownLatch(1);
    ...
    @ParameterizedTest
    @MethodSource("data")
    public void enableAccSampling(Class<? extends Accelerometer> accelClass) throws InterruptedException {
        CountDownLatch doneSignal = new CountDownLatch(1);
        setup(accelClass).addOnSuccessListener(IMMEDIATE_EXECUTOR, task -> {
            byte[] expected = null;
    ...
    @ParameterizedTest
    @MethodSource("data")
    public void disableAccSampling(Class<? extends Accelerometer> accelClass) throws InterruptedException {
        CountDownLatch doneSignal = new CountDownLatch(1);
        setup(accelClass).addOnSuccessListener(IMMEDIATE_EXECUTOR, task -> {
            byte[] expected = null;
    ...
    @ParameterizedTest
    @MethodSource("data")
    public void receiveAccData(Class<? extends Accelerometer> accelClass) throws InterruptedException {
        CountDownLatch doneSignal = new CountDownLatch(1);
        setup(accelClass).addOnSuccessListener(IMMEDIATE_EXECUTOR, ignored -> {
            ...
            Acceleration finalExpected = expected;
            accelerometer.acceleration().addRouteAsync(source -> source.stream((data, env) ->
                    ...
                    .addOnSuccessListener(IMMEDIATE_EXECUTOR, task -> {
                        ...
                        sendMockResponse(finalResponse);

                        assertEquals(finalExpected, actual.get());
                        doneSignal.countDown();
                    });
        });
        doneSignal.await(TEST_WAIT_TIME, TimeUnit.SECONDS);
    ...
    @ParameterizedTest
    @MethodSource("data")
    public void receiveSingleAxisAccData(Class<? extends Accelerometer> accelClass) throws InterruptedException {
        CountDownLatch doneSignal = new CountDownLatch(1);
        setup(accelClass).addOnSuccessListener(IMMEDIATE_EXECUTOR, ignored -> {
            float[] expected = null;
            byte[] response = null;
            ...
            accelerometer.acceleration().addRouteAsync(source -> source.split()
                    ...
                    .addOnSuccessListener(IMMEDIATE_EXECUTOR, task -> {
                        ...
                        assertArrayEquals(finalExpected, actual, 0.001f);
                        doneSignal.countDown();
                    });
        });
        doneSignal.await(TEST_WAIT_TIME, TimeUnit.SECONDS);
    ...
    @ParameterizedTest
    @MethodSource("data")
    public void receivedPackedData(Class<? extends Accelerometer> accelClass) throws InterruptedException {
        CountDownLatch doneSignal = new CountDownLatch(1);
        setup(accelClass).addOnSuccessListener(IMMEDIATE_EXECUTOR, ignored -> {
            Acceleration[] expected = null;
            byte[] response = null;
