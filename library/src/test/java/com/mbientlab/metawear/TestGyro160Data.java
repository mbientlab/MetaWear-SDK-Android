...
public class TestGyro160Data extends UnitTestBase {
    private Gyro gyro;
    ...
    @Test
    public void interpretComponentData() throws InterruptedException {
        CountDownLatch doneSignal = new CountDownLatch(1);
    ...
    public void unsubscribe() throws InterruptedException {
        ...
        setup().addOnSuccessListener(IMMEDIATE_EXECUTOR, ignored -> {
            gyro.angularVelocity().addRouteAsync(source -> source.stream(null)).addOnSuccessListener(IMMEDIATE_EXECUTOR, task -> {
                task.unsubscribe(0);
                assertArrayEquals(expected, junitPlatform.getLastCommand());
                doneSignal.countDown();
            });
