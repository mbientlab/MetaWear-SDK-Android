...
public class SensorFusionEulerAnglesMessage extends Message {
    private final float heading, pitch, roll, yaw;

    public SensorFusionEulerAnglesMessage(byte[] data) {
        this(null, data);
    }

    public SensorFusionEulerAnglesMessage(Calendar timestamp, byte[] data) {
    ...
    public <T> T getData(Class<T> type) {
        if (type.equals(EulerAngle.class)) {
            return type.cast(new EulerAngle() {
                @Override
                public String toString() {
                    return String.format(Locale.US, "{heading: %.3f, pitch: %.3f, yaw: %.3f, roll: %.3f}",
                            heading(), pitch(), yaw(), roll());
                }
